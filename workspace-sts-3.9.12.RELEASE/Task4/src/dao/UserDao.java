...
public class UserDao {
	...
	public static List<User> getAllUsers(){
		...
		try {
			...
			while(rs.next()) {
				...
				user.setPhoneNumber(rs.getNString(6));
				user.setEmail(rs.getString(7));
				user.setNumberOfIDCard(rs.getString(8));
				user.setBloodType(rs.getString(8));
				user.setSex(rs.getString(9));
				user.setNameOfJob(rs.getString(10));
	...
	public static User getUserById(int id) {
		...
		try {
			Connection con = ConnectionDao.getConnection();
			PreparedStatement ps = con.prepareStatement("select * from tblUsers where UserId=?");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				user.setUserID(rs.getInt(1));
